import React from "react"

interface IconStartProps {
  width: number
  height: number
}

const IconStart = ({ width, height }: IconStartProps) => {
  return (
    <svg
      width={width}     
      height={height}
      viewBox="0 0 56.693 56.693"
      id="arduino_simulator_start_icon"
    >
      <path fill="green" d="M46.279,26.484L20.113,7.48c-0.957-0.695-2.228-0.796-3.278-0.258c-1.056,0.537-1.721,1.622-1.721,2.807  v38.009c0,1.184,0.665,2.268,1.721,2.805c0.45,0.23,0.939,0.344,1.429,0.344c0.652,0,1.303-0.203,1.85-0.602l26.166-19.006  c0.818-0.594,1.298-1.539,1.298-2.548C47.577,28.023,47.094,27.077,46.279,26.484z" />
    </svg>
  )
}

export default IconStart
